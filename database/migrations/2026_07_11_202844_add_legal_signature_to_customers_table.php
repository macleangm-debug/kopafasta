    ...
    public function up(): void
    {
        Schema::table('customers', function (Blueprint $table) {
            if (! Schema::hasColumn('customers', 'legal_signature_data')) {
                $table->longText('legal_signature_data')->nullable()->after('face_verification_status');
            }
            if (! Schema::hasColumn('customers', 'legal_signer_name')) {
                $table->string('legal_signer_name')->nullable()->after('legal_signature_data');
            }
            if (! Schema::hasColumn('customers', 'legal_signed_at')) {
                $table->timestamp('legal_signed_at')->nullable()->after('legal_signer_name');
            }
        });
    }
