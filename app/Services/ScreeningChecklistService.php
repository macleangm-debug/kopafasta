...
class ScreeningChecklistService
{
    ...
    private function formatCapacity(array $eval, array $policy, float $proposed): array
    {
        ...
        $assessment = match (true) {
            ...
            $within && $delta >= 0 => 'Evidence supports a maximum repayment of '
                .format_money($max).' against the required '.format_money($proposed).'.',
            default => 'Required repayment '.format_money($proposed)
                .' exceeds the evidence-supported maximum of '.format_money($max).'.',
        };
        ...
        return [
            ...
            'result' => $within ? '✓ Within affordability limit' : '✕ Above affordability limit',
