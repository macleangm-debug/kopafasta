...
<x-admin.table-shell :records="$rows" :statuses="$statuses" searchPlaceholder="Search application #, customer…">
    <x-slot:headers>
        ...
        <x-admin.th :sort="$sort" :direction="$direction" col="amount_requested"   label="Amount" />
    ...
    <x-slot:rows>
        @forelse ($rows as $r)
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-5 py-3">TZS {{ number_format((float) ($r->amount_requested ?? 0)) }}</td>
                <td class="px-5 py-3">
                ...
                <td class="px-5 py-3 text-right">
                    <button class="text-xs font-medium text-amber-600 hover:text-amber-700">View</button>
                </td>
            </tr>
        @empty
