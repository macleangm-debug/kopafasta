    ...
    <x-slot:rows>
        @forelse ($rows as $r)
            ...
            <tr class="hover:bg-gray-50">
                <td class="px-5 py-3 font-medium"><a href="{{ route('admin.pep-flags.show', $r) }}" class="hover:text-brand">{{ $r->full_name }}</a></td>
                ...
                <td class="px-5 py-3"><span class="inline-flex items-center rounded px-2 py-0.5 text-xs {{ $r->is_active ? 'bg-brand-muted text-brand' : 'bg-gray-100 text-gray-600' }}">{{ $r->is_active ? 'Flagged' : 'Cleared' }}</span></td>
            </tr>
        @empty
