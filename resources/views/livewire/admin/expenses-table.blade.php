...
<x-admin.table-shell :records="$rows" :statuses="$statuses" searchPlaceholder="Search description, reference, category…">
    <x-slot:headers>
        ...
        <th class="px-5 py-2.5 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">Actions</th>
    </x-slot:headers>
    <x-slot:rows>
        @php $categoryService = app(\App\Services\OperationalExpenseCategoryService::class); @endphp
        @forelse ($rows as $r)
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-5 py-3">
                    <div class="font-medium">{{ $categoryService->labelFor((string) ($r->category ?? '')) }}</div>
                ...
                <td class="px-5 py-3 text-right">
                    <a href="{{ route('admin.expenses.show', $r) }}" class="text-xs font-medium text-brand hover:text-brand-light">View →</a>
                </td>
            </tr>
        @empty
            <tr><td colspan="7" class="px-5 py-12 text-center text-gray-500">No operational expenses found.</td></tr>
        @endforelse
    </x-slot:rows>
</x-admin.table-shell>
</div>
