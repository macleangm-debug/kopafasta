<div>
<x-admin.table-shell :records="$rows" searchPlaceholder="Search repayment method…">
    <x-slot:headers>
        ...
        <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Auto reconcile</th>
    ...
    <x-slot:rows>
        @forelse ($rows as $r)
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-5 py-3 font-medium"><a href="{{ route('admin.repayment-methods.show', $r) }}" class="hover:text-brand">{{ $r->name }}</a></td>
        ...
        @empty
            <tr><td colspan="5" class="px-5 py-12 text-center text-gray-500">No methods yet.</td></tr>
        @endforelse
    </x-slot:rows>
</x-admin.table-shell>
</div>
