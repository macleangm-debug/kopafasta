...
<div x-show="stepKey === 'valuation_fee'" class="p-6 sm:p-8">
    <h2 class="text-xl font-semibold mb-1">{{ __('borrower.apply.valuation_fee.title') }}</h2>
    <p class="text-sm text-gray-600 mb-5">{{ __('borrower.apply.valuation_fee.subtitle') }}</p>
    ...
    <div x-show="valuationFeeState?.status === 'pending'" x-cloak class="rounded-xl bg-sky-50 ring-1 ring-sky-200 px-4 py-4 text-sm text-sky-900 mb-6">
    ...
    <div x-show="valuationFeePaid" x-cloak class="rounded-xl bg-emerald-50 ring-1 ring-emerald-200 px-4 py-4 text-sm text-emerald-900 mb-6">
    ...
    <div x-show="!valuationFeePaid && effectiveValuationFeeAmount() <= 0" x-cloak class="rounded-xl bg-gray-50 ring-1 ring-gray-200 px-4 py-4 text-sm text-gray-700 mb-6">
        {{ __('borrower.apply.valuation_fee.waived') }}
    </div>

    <div x-show="showsValuationFeePayment()" x-cloak>
        <div class="rounded-2xl bg-gradient-to-br from-sky-500 to-sky-600 text-white p-6 shadow-lg mb-6">
            ...
            @endif
            <p class="mt-1 text-3xl font-extrabold">{{ $currency }} <span x-text="formatTzs(effectiveValuationFeeAmount())"></span></p>
            @if ($paymentReference)
        ...
        <div class="space-y-5">
            <div>
                ...
                <div class="grid sm:grid-cols-2 gap-3">
                    <label class="cursor-pointer">
                        ...
                        <div class="rounded-xl border-2 border-gray-200 peer-checked:border-sky-500 peer-checked:bg-sky-50 p-4">
            ...
            <div x-show="valuationFeeChannel === 'mobile_money'" x-transition>
                ...
                <input type="tel" x-model="valuationFeePhone" class="w-full rounded-lg border-gray-300 px-3 py-2.5 text-sm" placeholder="+255 7XX XXX XXX">
            </div>
