...
<div class="space-y-3" x-data="documentUpload(@js($disabled), @js($multiple))">
    @unless($disabled)
        <div class="flex flex-wrap gap-2">
            <button type="button" @click="mode='camera'" :class="mode === 'camera' ? activeTab : idleTab">Camera</button>
            <button type="button" @click="mode='gallery'" :class="mode === 'gallery' ? activeTab : idleTab">Gallery</button>
            <button type="button" @click="mode='pdf'" :class="mode === 'pdf' ? activeTab : idleTab">PDF</button>
        </div>
        ...
        <div x-show="mode === 'camera'" x-cloak class="space-y-3">
            ...
            <div class="flex flex-wrap gap-2">
                <button type="button" @click="startCamera()" class="text-sm font-semibold px-4 py-2 rounded-full bg-gray-900 text-white">Start camera</button>
                <button type="button" @click="capture()" class="text-sm font-semibold px-4 py-2 rounded-full bg-amber-500 text-gray-900">Capture page</button>
            </div>
        </div>
        ...
        <div x-show="queued.length" class="space-y-2">
            <p class="text-xs font-semibold text-gray-500 uppercase tracking-widest">Ready to upload (<span x-text="queued.length"></span>)</p>
            <template x-for="(item, index) in queued" :key="index">
                <div class="flex items-center justify-between gap-2 text-sm bg-gray-50 ring-1 ring-gray-200 rounded-lg px-3 py-2">
                    ...
                    <button type="button" @click="removeQueued(index)" class="text-red-600 text-xs font-semibold">Remove</button>
                </div>
            </template>
        </div>
        ...
        <form x-ref="form" method="POST" action="{{ $action }}" enctype="multipart/form-data" @submit.prevent="submitForm">
            ...
            @if ($showClarification)
                <div class="mb-3">
                    <label class="block text-xs font-semibold text-gray-600 mb-1">Your response</label>
                    <textarea name="response" rows="3" class="w-full rounded-xl border-gray-200 text-sm" placeholder="Type your clarification here…"></textarea>
                </div>
            @endif
    ...
    @else
        <p class="text-sm text-gray-500">No further action required.</p>
    @endunless
</div>
