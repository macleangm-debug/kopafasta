...
@php
    ...
    $na = $t('NOT APPLICABLE', 'HAITUMIKI');
    $graceDays = (int) ($snapshot['grace_days'] ?? 7);
    $penaltyCap = format_number($snapshot['penalty_cap_percent'] ?? 30, 0);
@endphp
...
@if ($isSw)
    <p>Mkopeshaji ni <strong>{{ $company }}</strong>, yenye leseni/usajili wa kutoa huduma za microfinance nchini Tanzania (“Mkopeshaji”, “Kopafasta”, “sisi”).</p>
@else
    <p>The lender is <strong>{{ $company }}</strong>, licensed/registered to provide microfinance services in the United Republic of Tanzania (“Lender”, “Kopafasta”, “we”).</p>
@endif
<table class="kv">
...
<table class="kv">
    ...
    <tr><td class="label">{{ $t('Activity', 'Shughuli') }}</td><td class="value">{{ $snapshot['customer_activity'] ?? '—' }}</td></tr>
...
<table class="kv">
    ...
    <tr><td class="label">{{ $t('Relationship', 'Uhusiano') }}</td><td class="value">{{ $snapshot['guarantor_relationship'] ?? '—' }}</td></tr>
</table>
@else
<p class="na">{{ $na }}</p>
@endif
...
<table class="kv">
    ...
    @endif
    <tr><td class="label">{{ $t('Purpose', 'Madhumuni') }}</td><td class="value">{{ $snapshot['purpose'] ?? '—' }}</td></tr>
</table>
...
@if ($isSw)
    <p>Mfano: ikiwa TZS 100,000 msingi, TZS 10,000 riba na TZS 5,000 adhabu zinadaiwa na Mkopaji analipa TZS 100,000, malipo yataondoa kwanza adhabu TZS 5,000, kisha riba TZS 10,000, kisha msingi TZS 85,000, na TZS 15,000 ya msingi itabaki.</p>
@else
    <p>Example: if TZS 100,000 principal, TZS 10,000 interest and TZS 5,000 penalty are due and the Borrower pays TZS 100,000, the payment first clears the TZS 5,000 penalty, then TZS 10,000 interest, then TZS 85,000 principal, leaving TZS 15,000 principal outstanding.</p>
@endif
...
@if ($isSw)
    ...
    <p>Adhabu ni tofauti na gharama yoyote ya urejeshaji. Mfano: awamu ya TZS 50,000 isiyolipwa kwa 1% kwa siku baada ya msamaha wa siku 3 ni TZS 500 siku ya kwanza ya adhabu kwenye salio la awamu hiyo, kisha TZS 1,000 jumla siku inayofuata, kulingana na kizuizi cha {{ $penaltyCap }}% cha salio zote zilizochelewa.</p>
@else
    ...
    <p>Penalty is separate from any recovery charge. Example: an unpaid instalment of TZS 50,000 at 1% per day after a 3-day grace is TZS 500 on the first penalty day on that instalment remainder, then TZS 1,000 cumulative on the next day, subject to the {{ $penaltyCap }}% cap on all overdue remainders.</p>
@endif
...
@if (! empty($snapshot['is_group_loan']))
...
<h2>{{ $t('26–30. No waiver, severability, entire agreement, governing law, language', '26–30. Kutotumia haki, utenganishaji, mkataba mzima, sheria inayotumika, lugha') }}</h2>
@if ($isSw)
    <p>Kuchelewa au kushindwa kutumia haki si kuiacha. Iwapo kifungu kimoja ni batili, vingine vinaendelea. Mkataba huu, Barua ya Ofa, Jedwali la Huduma na viambatisho ndio makubaliano kuhusu Mkopo. Unatawaliwa na sheria za <strong>{{ $snapshot['jurisdiction'] ?? 'Jamhuri ya Muungano wa Tanzania' }}</strong>. Toleo hili limetolewa katika lugha uliyochagua. Toleo la lugha nyingine lina maana ileile; iwapo kuna tofauti, <strong>toleo la Kiingereza ndilo litakalotawala</strong> kwa kiwango kinachoruhusiwa na sheria.</p>
@else
    <p>Delay or failure to exercise a right is not a waiver. If a provision is invalid, the rest continues. This Agreement, the Offer Letter, Facility Schedule and annexes are the agreement concerning the Loan. It is governed by the laws of <strong>{{ $snapshot['jurisdiction'] ?? 'United Republic of Tanzania' }}</strong>. This version is issued in the language you selected. The other language version has the same meaning; if there is an inconsistency, the <strong>English version prevails</strong> to the extent permitted by law.</p>
@endif
...
<div class="annex">
    ...
    <table class="charges">
        ...
        @foreach ($stages as $stage)
            <tr><td>{{ $stage['label'] }}</td><td>{{ $isSw ? ($stage['display_sw'] ?? '') : ($stage['display_en'] ?? '') }}</td></tr>
        @endforeach
    </table>
