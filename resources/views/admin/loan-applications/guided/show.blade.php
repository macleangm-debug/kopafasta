...
<x-admin.guided-review-shell
    ...
    :overallRemaining="$guided['remaining_blockers'] ?? null"
    ...
    backLabel="Back to Screening">
        ...
        @if (count($guided['subjects'] ?? []) > 1)
            <div class="mt-4 grid gap-2 sm:grid-cols-2" data-participant-switcher>
                @foreach ($guided['subjects'] as $subject)
                    @php
                        ...
                        $onGate3 = ($step['gate'] ?? '') === 'crb' || ($type ?? '') === 'gate_3_passed';
                        $mark = $onGate3
                            ? (string) ($subject['gate3']['chip'] ?? '')
                            : (string) ($subject['gate2']['chip'] ?? '');
                        if ($mark === '') {
                            $mark = ! empty($subject['complete'])
                                ? 'PASSED'
                                : ((int) ($subject['failed'] ?? 0) > 0 ? 'REFER' : 'WAITING');
                        }
                        $role = (string) ($subject['role'] ?? match ($subject['person'] ?? '') {
                            'guarantor' => 'Guarantor',
                            'member' => 'Member',
                            default => 'Borrower',
                        });
                        $gateChipLabel = $onGate3 ? 'Gate 3' : 'Gate 2';
                    @endphp
                    <a href="{{ $subject['href'] }}"
                       @class([
                           'rounded-2xl px-3.5 py-2.5 ring-1 text-left transition',
                           'bg-brand text-white ring-brand shadow-sm' => $current,
                           'bg-white text-slate-800 ring-slate-200 hover:ring-brand/40' => ! $current,
                       ])>
                        <p class="text-sm font-bold leading-snug break-words">{{ $subject['label'] ?: 'Participant' }}</p>
                        <p @class([
                            'text-[11px] font-semibold mt-0.5',
                            'text-white/85' => $current,
                            'text-slate-500' => ! $current,
                        ])>{{ $role }} · {{ $gateChipLabel }} · {{ $mark }}</p>
                    </a>
                @endforeach
            </div>
        @endif
        ...
        @if (($step['gate'] ?? '') === 'income'
            && ($type ?? '') !== 'gate_2_passed'
            && ! empty($guided['gate2_summary']['rows']))
            <div class="mt-3 rounded-2xl bg-slate-50 ring-1 ring-slate-200 px-4 py-3 space-y-2" data-gate2-summary>
                ...
                <ul class="grid gap-3 sm:grid-cols-2 text-sm">
                    @foreach ($guided['gate2_summary']['rows'] as $row)
                        <li class="rounded-xl bg-white ring-1 ring-slate-200/80 px-3 py-2 space-y-1">
                            <div class="flex items-center justify-between gap-2">
                                <span class="font-semibold text-slate-900 break-words">{{ $row['name'] }}</span>
                                <span class="shrink-0 font-bold {{ ($row['chip'] ?? '') === 'PASSED' ? 'text-emerald-800' : (($row['chip'] ?? '') === 'FAILED' ? 'text-rose-800' : 'text-amber-800') }}">{{ $row['chip'] }}</span>
                            </div>
                            @if (! empty($row['checks']))
                                <ul class="space-y-0.5 text-xs text-slate-700">
                                    @foreach ($row['checks'] as $check)
                                        <li class="flex justify-between gap-2">
                                            ...
                                            <span class="font-bold {{ ($check['chip'] ?? '') === 'PASSED' ? 'text-emerald-800' : (($check['chip'] ?? '') === 'FAILED' ? 'text-rose-800' : 'text-amber-800') }}">{{ $check['chip'] ?? 'WAITING' }}</span>
                                        </li>
                                    @endforeach
                                </ul>
                            @endif
                ...
                @if ((int) ($guided['gate2_summary']['gate_remaining'] ?? 0) > 0)
                    <p class="text-xs font-semibold text-amber-900">{{ (int) $guided['gate2_summary']['gate_remaining'] }} {{ (int) $guided['gate2_summary']['gate_remaining'] === 1 ? 'check remains' : 'checks remain' }} in Gate 2</p>
                @endif
            </div>
        @endif
        ...
        @if (($step['gate'] ?? '') === 'crb'
            && ($type ?? '') !== 'gate_3_passed'
            && ! empty($guided['gate3_summary']['rows']))
            <div class="mt-3 rounded-2xl bg-slate-50 ring-1 ring-slate-200 px-4 py-3 space-y-2" data-gate3-summary>
                <p class="text-[10px] uppercase tracking-widest text-slate-500 font-bold">Gate 3 · Participants</p>
                <ul class="space-y-1.5 text-sm">
                    @foreach ($guided['gate3_summary']['rows'] as $row)
                        <li class="flex flex-wrap items-center justify-between gap-2">
                            <a href="{{ $row['href'] ?? '#' }}" class="font-semibold text-slate-900 underline-offset-2 hover:underline">
                                {{ $row['name'] }}@if (! empty($row['role'])) · {{ $row['role'] }}@endif
                            </a>
                            <span class="font-bold {{ ($row['chip'] ?? '') === 'PASSED' ? 'text-emerald-800' : (($row['chip'] ?? '') === 'FAILED' ? 'text-rose-800' : 'text-amber-800') }}">{{ $row['chip'] ?? 'WAITING' }}</span>
                        </li>
                    @endforeach
                </ul>
                <p class="text-xs font-semibold text-slate-800 border-t border-slate-200 pt-2">
                    GATE 3 · {{ $guided['gate3_summary']['overall'] ?? 'WAITING' }}
                </p>
                @if (! empty($guided['gate3_summary']['continue']))
                    <p class="text-xs text-slate-700">{{ $guided['gate3_summary']['continue'] }}</p>
                @endif
            </div>
        @endif

        @if (! in_array($type, ['human', 'request', 'attention', 'gate_2_passed', 'gate_3_passed'], true))
            <h2 class="text-lg font-bold text-slate-900 mt-5 break-words">{{ $step['title'] ?? $guided['cta'] }}</h2>
        @endif

        @if (! empty($guided['what_happens_next']) && ! in_array($type ?? '', ['gate_2_passed', 'gate_3_passed'], true))
        ...
        @elseif ($type === 'gate_2_passed')
            <div class="mt-5 rounded-2xl bg-white ring-1 ring-brand/15 px-4 py-5 space-y-4">
                <div class="flex flex-wrap items-center justify-between gap-2">
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-widest text-brand">Gate 2 · Verified affordability</p>
                        <p class="text-lg font-bold text-emerald-800 mt-1">✓ PASSED</p>
                    </div>
                </div>
                <ul class="grid gap-2 sm:grid-cols-2 text-sm">
                    @foreach ($step['participants'] ?? [] as $row)
                        <li class="flex items-center justify-between gap-2 rounded-xl bg-slate-50 ring-1 ring-slate-200 px-3 py-2">
                            <span class="font-semibold text-slate-900 break-words">{{ $row['name'] }}</span>
                            <span class="shrink-0 font-bold {{ ! empty($row['pass']) ? 'text-emerald-800' : 'text-amber-800' }}">
                                {{ ! empty($row['pass']) ? 'PASSED' : 'WAITING' }}
                            </span>
                        </li>
                    @endforeach
                </ul>
                <p class="text-sm text-slate-700">{{ $step['prompt'] ?? 'All required participants passed Gate 2.' }}</p>
                <p class="text-xs text-slate-500">Next: CRB review.</p>
            </div>
        @elseif ($type === 'gate_3_passed')
            <div class="mt-5 rounded-2xl bg-white ring-1 ring-brand/15 px-4 py-5 space-y-4">
                <div>
                    <p class="text-[11px] font-bold uppercase tracking-widest text-brand">Gate 3 · CRB &amp; credit history</p>
                    <p class="text-lg font-bold text-emerald-800 mt-1">✓ PASSED</p>
                </div>
                <ul class="grid gap-2 sm:grid-cols-2 text-sm">
                    @foreach ($step['participants'] ?? [] as $row)
                        <li class="flex items-center justify-between gap-2 rounded-xl bg-slate-50 ring-1 ring-slate-200 px-3 py-2">
                            <span class="font-semibold text-slate-900 break-words">{{ $row['name'] }}</span>
                            <span class="shrink-0 font-bold {{ ! empty($row['pass']) ? 'text-emerald-800' : 'text-amber-800' }}">
                                {{ $row['chip'] ?? (! empty($row['pass']) ? 'PASSED' : 'WAITING') }}
                            </span>
                        </li>
                    @endforeach
                </ul>
                <p class="text-sm text-slate-700">{{ $step['prompt'] ?? 'All required participants cleared Gate 3.' }}</p>
                <p class="text-xs text-slate-500">Next: Gate 4 · Collateral. Gate 3 does not auto-advance.</p>
            </div>
        ...
        @elseif ($type === 'gate_remaining')
            <div class="mt-6 rounded-2xl bg-white ring-1 ring-brand/15 px-4 py-5 space-y-3">
                <p class="text-sm font-bold text-slate-900">{{ $step['title'] }}</p>
                <p class="text-sm text-slate-800">{{ $step['prompt'] }}</p>
                @if (! empty($step['people_remaining']))
                    <ul class="space-y-1 text-sm">
                        @foreach ($step['people_remaining'] as $name => $count)
                            <li class="flex justify-between gap-2">
                                <span class="font-semibold text-slate-900">{{ $name }}</span>
                                <span class="font-bold text-amber-800">{{ $count }} remaining</span>
                            </li>
                        @endforeach
                    </ul>
                @endif
                @if ((int) ($step['overall_remaining'] ?? 0) > (int) ($step['gate_remaining'] ?? 0))
                    <p class="text-xs text-slate-500">{{ (int) $step['overall_remaining'] }} checks remain overall across Screening. Finish this gate first.</p>
                @endif
            </div>
        @elseif ($type === 'attention')
            <div class="mt-2 space-y-4">
                ...
                <p class="text-sm text-slate-800">{{ $step['prompt'] }}</p>
                @if (($step['gate'] ?? '') === 'crb' && ! empty($step['gate3_panel']))
                    @include('admin.loan-applications.review._gate3_finding', [
                        'panel' => $step['gate3_panel'],
                        'record' => $record,
                    ])
                @endif
            ...
            <div class="mt-2 space-y-4">
                ...
                @if (empty($step['revisiting']) || blank($step['verdict'] ?? null))
                    ...
                    @endphp
                    <form method="POST" action="{{ route('admin.loan-applications.guided-screening.save', $record) }}"
                          id="guided-screening-form" class="space-y-3"
                          @if (! empty($capturesStatement) || ! empty($step['captures_statement']))
                              data-kf-autosave
                              data-kf-autosave-allow-submit
                              data-kf-autosave-debounce-ms="800"
                          @else
                              data-no-draft
                          @endif
                        ...
                        @if (! empty($participant['g']))
                            <input type="hidden" name="g" value="{{ $participant['g'] }}">
                        @endif
                        @php $capturesStatement = ! empty($step['captures_statement']); @endphp
                        @if ($capturesStatement)
                            @php
                                $capacityPreview = is_array($step['capacity'] ?? null) ? $step['capacity'] : [];
                                $ratioPctLive = (float) ($capacityPreview['ratio_pct'] ?? app(\App\Services\AffordabilityPolicyService::class)->repaymentRatioPct());
                                $requiredLive = (float) ($capacityPreview['proposed_repayment'] ?? 0);
                                $requiredLabelLive = (string) ($capacityPreview['required_label'] ?? 'Required loan repayment');
                                $cadenceLive = (string) ($capacityPreview['repayment_cadence'] ?? 'monthly');
                            @endphp
                            <div class="rounded-xl ring-1 ring-brand/15 bg-brand-muted/30 p-3.5 space-y-3"
                                 x-data="{
                                    ...
                                    ratioPct: @js($ratioPctLive),
                                    required: @js($requiredLive),
                                    digits() { return Number(String(this.raw).replace(/[^0-9.]/g, '')) || 0 },
                                    monthly() { return this.digits() / 6 },
                                    maxSupported() { return this.monthly() * (this.ratioPct / 100) },
                                    delta() { return this.maxSupported() - this.required },
                                    money(n) { return 'TZS ' + Math.round(Math.abs(n)).toLocaleString('en-US') },
                                    signedMoney(n) { return (n < 0 ? '-' : '') + this.money(n) }
                                 }">
                                ...
                                <dl class="grid sm:grid-cols-2 gap-2 text-sm">
                                    ...
                                    <div>
                                        <dt class="text-[10px] uppercase tracking-widest text-slate-500">Verified monthly income</dt>
                                        <dd class="font-semibold" x-text="money(monthly())"></dd>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <dd class="font-semibold"
                                            x-text="digits() > 0
                                                ? ('Verified statement average · ' + money(monthly()))
                                                : 'Waiting for statement total'"></dd>
                                    </div>
                                </dl>

                                <div data-statement-comparison-live x-show="digits() > 0 && declared > 0" x-cloak class="rounded-lg bg-white px-3 py-2 ring-1 ring-brand/10 space-y-2">
                                    ...
                                    <dl class="grid sm:grid-cols-2 gap-2 text-sm">
                                        ...
                                        <div>
                                            ...
                                            <dd class="font-semibold" x-text="signedMoney(monthly() - declared)"></dd>
                                        </div>
                                        <div>
                                            <dt class="text-[10px] uppercase tracking-widest text-slate-500">Statement coverage</dt>
                                            <dd class="font-semibold" x-text="((monthly() / declared) * 100).toFixed(1) + '%'"></dd>
                                        </div>
                                    </dl>
                                    <p class="text-sm text-slate-800" x-text="monthly() < declared
                                        ? 'Statement activity is below the declared monthly income. This does not by itself fail Gate 2.'
                                        : 'Statement activity covers the declared monthly income. Capacity uses verified income below.'"></p>
                                </div>

                                {{-- Live capacity preview while typing; server panel below is authoritative after save --}}
                                <div data-capacity-live x-show="digits() > 0 && required > 0 && Math.abs(monthly() - storedMonthly) > 1" x-cloak
                                     class="rounded-xl ring-1 ring-brand/20 bg-white px-3.5 py-3 space-y-3">
                                    <div class="flex flex-wrap items-center justify-between gap-2">
                                        <p class="text-[10px] uppercase tracking-widest text-brand font-bold">Verified repayment capacity</p>
                                        <span class="inline-flex items-center rounded-lg px-2.5 py-1 text-xs font-bold ring-1"
                                              :class="delta() >= 0 ? 'bg-emerald-50 text-emerald-900 ring-emerald-200' : 'bg-rose-50 text-rose-950 ring-rose-200'"
                                              x-text="delta() >= 0 ? '✓ CAPACITY PASSED' : '✕ CAPACITY FAILED'"></span>
                                    </div>
                                    <dl class="grid sm:grid-cols-2 gap-2 text-sm">
                                        <div>
                                            <dt class="text-[10px] uppercase tracking-widest text-slate-500">Verified monthly income</dt>
                                        ...
                                        <div>
                                            <dt class="text-[10px] uppercase tracking-widest text-slate-500">Maximum repayment ratio</dt>
                                            <dd class="font-semibold">{{ rtrim(rtrim(number_format($ratioPctLive, 2, '.', ''), '0'), '.') }}%</dd>
                                        </div>
                                        <div>
                                            <dt class="text-[10px] uppercase tracking-widest text-slate-500">Maximum supported repayment</dt>
                                            <dd class="font-semibold" x-text="money(maxSupported())"></dd>
                                        </div>
                                        <div>
                                            <dt class="text-[10px] uppercase tracking-widest text-slate-500">{{ $requiredLabelLive }}</dt>
                                            <dd class="font-semibold">{{ format_money($requiredLive) }} / {{ $cadenceLive }}</dd>
                                        </div>
                                        <div>
                                            <dt class="text-[10px] uppercase tracking-widest text-slate-500" x-text="delta() >= 0 ? 'Headroom' : 'Shortfall'"></dt>
                                            <dd class="font-semibold" x-text="money(Math.abs(delta()))"></dd>
                                        </div>
                                    </dl>
                                    <p class="text-sm text-slate-800"
                                       x-text="delta() >= 0
                                         ? 'Evidence supports the required repayment.'
                                         : 'Verified income does not support the required repayment.'"></p>
                                    <p class="text-[11px] text-slate-500">Preview uses the Settings ratio. Exact figures refresh from the affordability service after Save.</p>
                                </div>

                                @include('admin.loan-applications.review._gate2_finding', [
                                    'panel' => (($step['income_basis'] ?? '') === 'statement' || (float) ($step['statement_monthly'] ?? 0) > 0)
                                        ? ($step['gate2_panel'] ?? null)
                                        : null,
                                    ...
                                    'gate2Checks' => $step['gate2_checks'] ?? null,
                                ])
    ...
    <div class="flex gap-2 items-stretch">
            ...
            @elseif (($step['type'] ?? '') === 'gate_remaining')
                <a href="{{ route('admin.loan-applications.guided-screening', $record) }}" class="flex-[2] min-w-0 text-center rounded-xl bg-brand text-white font-bold text-sm py-3 px-2 leading-snug whitespace-normal">
                    {{ $step['primary'] ?? 'Continue Reviewing' }}
                </a>
            ...
            @elseif (($step['type'] ?? '') === 'gate_2_passed' && ! empty($step['all_pass']))
                <form method="POST" action="{{ route('admin.loan-applications.guided-screening.save', $record) }}" class="flex-[2] min-w-0" data-no-draft>
                    @csrf
                    <input type="hidden" name="ack_gate" value="income">
                    <button type="submit" class="w-full rounded-xl bg-brand text-white font-bold text-sm py-3 px-2 leading-snug whitespace-normal">Continue to<br class="sm:hidden"> Gate 3</button>
                </form>
            @elseif (($step['type'] ?? '') === 'gate_3_passed' && ! empty($step['all_pass']))
                <form method="POST" action="{{ route('admin.loan-applications.guided-screening.save', $record) }}" class="flex-[2] min-w-0" data-no-draft>
                    @csrf
                    <input type="hidden" name="ack_gate" value="crb">
                    <button type="submit" class="w-full rounded-xl bg-brand text-white font-bold text-sm py-3 px-2 leading-snug whitespace-normal">Continue to<br class="sm:hidden"> Gate 4</button>
                </form>
