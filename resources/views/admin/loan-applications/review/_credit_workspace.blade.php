            ...
            @elseif ($workspace === 'overview')
                @php
                    ...
                    $managementNext = ($isManagementApprovalStage || $isPostApprovalStage)
                        ...
                        : null;
                    $capacityParked = app(\App\Services\CapacityAutoRejectService::class)->isPending($record)
                        || (is_array($screeningReadiness ?? null)
                        && (
                            ($screeningReadiness['pending_rejection'] ?? false)
                            || ($screeningReadiness['status'] ?? '') === 'pending_rejection'
                            || (($screeningReadiness['decision_status']['state'] ?? '') === 'pending_rejection')
                        ));
                @endphp
                <div class="rounded-2xl {{ $capacityParked ? 'bg-rose-50 ring-rose-200' : 'bg-brand-muted/40 ring-brand/10' }} ring-1 px-4 py-3.5 space-y-3">
                    <div class="flex flex-wrap items-center justify-between gap-3">
                        <div>
                            <p class="text-sm font-semibold {{ $capacityParked ? 'text-rose-950' : 'text-gray-900' }}">
                                @if ($capacityParked)
                            ...
                            @if ($capacityParked)
                                <p class="text-sm text-rose-900 mt-1">
                                    {{ $screeningReadiness['pending_rejection_detail'] ?? $screeningReadiness['detail'] ?? 'Declared affordability failed. The system will re-evaluate before final rejection.' }}
                                </p>
                                <p class="text-xs text-rose-800/80 mt-0.5">
                                    Profile, NIDA, guarantor and CRB facts may still appear below — they are not the guided next action while this park is active.
                                </p>
                            @elseif ($guidedNext)
                        ...
                        @if ($capacityParked)
                            <div class="flex flex-col items-stretch sm:items-end gap-2">
                                <span class="inline-flex rounded-xl bg-rose-900 text-white text-sm font-bold px-4 py-2.5">
                                    Waiting for scheduled re-evaluation
                                </span>
                                <a href="{{ route('admin.loan-applications.pipeline.system-sorted') }}" class="text-xs font-semibold text-rose-800 underline text-center sm:text-right">Open Automatic Rejections queue</a>
                            </div>
                        @elseif ($guidedNext)
                            <div class="flex flex-col items-stretch sm:items-end gap-2">
