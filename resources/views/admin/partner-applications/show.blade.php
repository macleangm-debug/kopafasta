<x-admin.layout
        ...
        <div class="bg-gradient-to-br from-brand via-brand to-brand-light px-5 sm:px-6 py-5 text-white">
            <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                <div class="flex items-start gap-4 min-w-0">
                    ...
                    <div class="min-w-0">
                        <p class="text-[10px] uppercase tracking-[0.2em] text-brand-gold font-semibold">{{ brand_name() }} · Partner enrolment file</p>
    ...
    <div class="grid lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            ...
            <x-admin.review-section id="review-checklist" title="Enrolment checklist" subtitle="Required documents for this partner category">
                <div class="grid sm:grid-cols-2 gap-3">
                    @forelse ($checklist as $item)
                        <div class="flex items-center justify-between gap-3 rounded-lg ring-1 px-4 py-3 text-sm {{ $item['present'] ? 'bg-emerald-50 ring-emerald-200 text-emerald-900' : 'bg-amber-50 ring-amber-200 text-amber-900' }}">
                            <span class="font-medium">{{ $item['label'] }}</span>
                            <span class="text-xs font-semibold rounded-full px-2 py-0.5 {{ $item['present'] ? 'bg-emerald-100' : 'bg-amber-100' }}">
                                {{ $item['present'] ? 'On file' : 'Missing' }}
                            </span>
                        </div>
                    @empty
                        <p class="text-sm text-gray-500">No specific documents are required for this category.</p>
                    @endforelse
                </div>
            </x-admin.review-section>

            <x-admin.review-section id="review-applicant" title="Applicant" subtitle="Contact details submitted with the application">
            ...
            <x-admin.review-section id="review-business" title="Business profile" subtitle="Registration details for verification">
                <dl class="grid sm:grid-cols-2 gap-4 text-sm">
                    <div>
            ...
            <x-admin.review-section id="review-identity" title="Identity verification" subtitle="National ID front and back">
                                ...
                                <button type="button"
                                    ...
                                    @if ($doc['is_image'])
                                        <img src="{{ $doc['url'] }}" alt="{{ $label }}"
                                             class="max-h-40 rounded-lg object-cover ring-1 ring-gray-200 group-hover:ring-amber-400 transition cursor-zoom-in">
                                    @else
                                    ...
                                    @endif
                                    <span class="text-xs font-semibold text-amber-700 mt-2 inline-block">Preview {{ $label }}</span>
                                </button>
                            @else
            ...
            <x-admin.review-section id="review-documents" title="Documents" subtitle="Every file submitted with this application">
                                    ...
                                    <button type="button"
                                        ...
                                        @if ($doc['is_image'])
                                            <img src="{{ $doc['url'] }}" alt="{{ $doc['label'] }}"
                                                 class="w-full h-32 rounded-lg object-cover ring-1 ring-gray-200 group-hover:ring-amber-400 transition cursor-zoom-in">
                                        @else
                                            <span class="flex h-32 items-center justify-center rounded-lg bg-gray-50 text-xs font-semibold text-gray-600 ring-1 ring-gray-200">📄 PDF document</span>
                                        @endif
                                        <span class="text-xs font-semibold text-amber-700 mt-2 inline-block">Preview</span>
                                    </button>
                                </div>
                            </div>
                        @endforeach
        ...
        <div class="space-y-6">
                ...
                <form method="POST" action="{{ route('admin.partner-applications.update', $application) }}" class="p-6 space-y-4"
                      ...
                      @submit.prevent="window.confirmForm($el, {
                          ...
                          message: status === 'approved'
                              ...
                              : (status === 'needs_info'
                                  ? 'The applicant will see your notes on the tracking page and can be asked to resubmit documents.'
                    ...
                    <div>
                        ...
                        <textarea name="admin_notes" rows="4"
                                  x-bind:placeholder="status === 'needs_info' ? 'Explain what is missing — the applicant will see this on the tracking page…' : 'Optional notes for the file…'"
            ...
            @if ($decision['reviewer'] || $decision['reviewed_at'])
                <div class="rounded-2xl bg-white shadow-sm ring-1 ring-gray-200 p-5 text-sm">
                    <p class="text-xs uppercase tracking-widest text-gray-500 font-semibold mb-2">Review history</p>
                    <p class="text-gray-700">
                        Last reviewed by <span class="font-semibold">{{ $decision['reviewer']?->name ?? '—' }}</span>
                        @if ($decision['reviewed_at'])
                            on {{ $decision['reviewed_at']->format('d M Y H:i') }}
                        @endif
                    </p>
                </div>
            @endif
