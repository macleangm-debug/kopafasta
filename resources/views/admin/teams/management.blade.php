<x-admin.layout title="Credit management" heading="" subheading="">
    ...
    <section class="mb-8">
        <div class="rounded-2xl overflow-hidden ring-1 ring-brand/15 shadow-sm">
            <div class="bg-gradient-to-br from-brand via-brand to-brand-light px-6 py-7 text-white">
                ...
                <h1 class="text-2xl sm:text-3xl font-bold mt-1">Credit management team</h1>
                <p class="text-sm text-white/75 mt-2 max-w-2xl">
                    Own the facility after committee approval — offer, fees, contract, push to disbursement, then servicing through arrears.
                </p>
            </div>
            <div class="bg-white px-6 py-5 grid sm:grid-cols-3 gap-4">
                <div class="rounded-xl bg-brand-muted/50 ring-1 ring-brand/10 px-4 py-4">
                    <p class="text-[10px] uppercase tracking-widest text-brand font-semibold">Post-approval</p>
                    <p class="text-3xl font-bold text-gray-900 mt-2 tabular-nums">{{ $counts['approved'] }}</p>
                    <p class="text-xs text-gray-500 mt-1">Awaiting offer / fees / contract</p>
                </div>
                <div class="rounded-xl bg-amber-50 ring-1 ring-amber-100 px-4 py-4">
                    <p class="text-[10px] uppercase tracking-widest text-amber-800 font-semibold">Ready / disbursement</p>
                    <p class="text-3xl font-bold text-gray-900 mt-2 tabular-nums">{{ $counts['disbursement_stage'] }}</p>
                    <p class="text-xs text-gray-500 mt-1">In the release pipeline</p>
                </div>
                <div class="rounded-xl bg-emerald-50 ring-1 ring-emerald-100 px-4 py-4">
                    <p class="text-[10px] uppercase tracking-widest text-emerald-800 font-semibold">Pending loans</p>
                    <p class="text-3xl font-bold text-gray-900 mt-2 tabular-nums">{{ $counts['pending_loans'] }}</p>
                    <p class="text-xs text-gray-500 mt-1">Awaiting payout on loan records</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <div class="rounded-2xl bg-white ring-1 ring-brand/10 shadow-sm p-5 sm:p-6">
        ...
        <p class="text-xs text-gray-500 mt-4">
            Spine: <span class="font-semibold text-gray-700">Offer → Post-approval fees → Contract → Disbursement → Active loan</span>.
            Screening and committee work stays on their desks. Destination and capital are not waited on here.
        </p>
    </div>
