<x-admin.layout title="Balance Sheet" heading="Balance Sheet" :subheading="'As at '.now()->format('Y-m-d')">
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm ring-1 ring-gray-200 p-6">
            ...
            <div class="mt-2 text-2xl font-bold text-brand font-mono">{{ format_number($equity, 2) }}</div>
        </div>
    </div>
    ...
    <div class="mt-6 bg-white rounded-xl shadow-sm ring-1 ring-gray-200 p-6 flex items-center justify-between">
        ...
        <span class="text-2xl font-bold {{ abs($diff) < 0.01 ? 'text-emerald-600' : 'text-brand' }} font-mono">{{ format_number($diff, 2) }}</span>
    </div>
</x-admin.layout>
