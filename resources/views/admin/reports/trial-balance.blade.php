        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($rows as $r)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-5 py-2 text-right font-mono">{{ $r->debit  ? number_format($r->debit, 2)  : '—' }}</td>
                        <td class="px-5 py-2 text-right font-mono">{{ $r->credit ? number_format($r->credit, 2) : '—' }}</td>
                    </tr>
                @empty
            ...
            <tfoot class="bg-gray-50 font-semibold">
                ...
                <tr class="{{ abs($totalDebit - $totalCredit) < 0.01 ? 'text-emerald-700' : 'text-rose-700' }}">
                    ...
                    <td class="px-5 py-3 text-right font-mono" colspan="2">{{ number_format($totalDebit - $totalCredit, 2) }}</td>
                </tr>
            </tfoot>
        </table>
    </div>
