...
<x-admin.layout
    ...
    <x-admin.letterhead
        ...
        :title="'Restructure request #'.$record->id"
        :subtitle="($loan?->loan_number ?? '—').' · '.$name">
        <x-slot:actions>
            <a href="{{ route('admin.restructure-requests.index') }}" class="inline-flex items-center text-xs font-semibold text-brand bg-brand-gold hover:brightness-95 px-3 py-1.5 rounded-lg">All requests</a>
        </x-slot:actions>
    </x-admin.letterhead>
