<x-admin.layout
    ...
    <div class="mb-4 flex flex-wrap gap-2">
        @foreach ([
            ...
            <a href="{{ route('admin.restructure-requests.index', ['status' => $key]) }}"
                    ...
                    @forelse ($requests as $request)
                        ...
                        <tr class="hover:bg-gray-50 align-top">
                            <td class="px-5 py-3 font-mono text-xs font-semibold">
                                <a href="{{ route('admin.restructure-requests.show', $request) }}" class="text-indigo-600 hover:text-indigo-700">
                                    {{ $loan?->loan_number ?? '—' }}
                                </a>
                            </td>
                            <td class="px-5 py-3">
                            ...
                            <td class="px-5 py-3 text-xs">{{ str_replace('_', ' ', $request->restructure_type ?? '—') }}</td>
                            <td class="px-5 py-3">
                            ...
                            <td class="px-5 py-3 text-right">
                                <a href="{{ route('admin.restructure-requests.show', $request) }}" class="text-indigo-600 font-semibold text-xs hover:underline">Review</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-5 py-10 text-center text-gray-500">No restructure requests found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
