<div class="bg-white rounded-xl shadow-sm ring-1 ring-gray-200 overflow-hidden mb-8">
    <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
        ...
        <span class="inline-flex items-center rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-semibold text-amber-800">
            {{ $pendingMembership->total() }} pending
        </span>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($pendingMembership as $payment)
                    @php
    ...
    @if ($pendingMembership->hasPages())
        <div class="px-5 py-3 border-t border-gray-100 bg-gray-50">
            {{ $pendingMembership->links() }}
        </div>
    @endif
</div>

@if (($recentMembership ?? collect())->isNotEmpty())
    <div class="bg-white rounded-xl shadow-sm ring-1 ring-gray-200 overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    @foreach ($recentMembership as $payment)
                        @php
                            $customer = $payment->customer;
                            $name = trim(($customer->first_name ?? '').' '.($customer->last_name ?? ''));
                        @endphp
