    ...
    <div class="relative overflow-hidden bg-slate-50">
        @if ($photoCount > 0)
            <div class="relative aspect-[4/3]">
                <img src="{{ $photoUrls[0] }}" alt="{{ $title }}" loading="lazy" decoding="async" referrerpolicy="no-referrer"
        ...
        @if (! empty($asset['max_tenure_months']))
            <span data-kf-duration-badge class="absolute bottom-3 left-3 rounded-lg bg-black/60 backdrop-blur text-white text-[11px] font-bold px-2.5 py-1 shadow-sm z-20 pointer-events-none">
