...
@if (($asset['supplier_deposit'] ?? 0) > 0)
    <div class="mt-3 rounded-xl bg-slate-50 ring-1 ring-slate-200 p-3 text-xs space-y-1">
        <p class="font-semibold text-gray-700 uppercase tracking-wide text-[10px]">Deposit breakdown</p>
        ...
        @endif
        <div class="flex justify-between pt-1 border-t border-slate-200"><span class="text-gray-700 font-semibold">Your deposit</span><span class="font-bold text-gray-900">{{ format_money($asset['deposit'] ?? 0) }}</span></div>
    </div>
@endif
