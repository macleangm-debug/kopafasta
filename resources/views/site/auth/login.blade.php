        ...
        <div class="h-full min-h-0 overflow-y-auto overscroll-y-contain flex items-center justify-center px-4 py-8 sm:px-12">
            <div id="login-method-switcher" class="w-full max-w-md glass-card p-6 sm:p-10" data-method="{{ $authMethod }}"
                ...
                <h1 class="text-3xl font-bold tracking-tight text-gray-900">
                    {{ $isPartnerPortal ? __('site.auth.partner_sign_in') : __('site.auth.welcome_back') }}
                </h1>
                ...
                @unless ($isPartnerPortal)
                    ...
                    <form method="POST" action="{{ route('site.login.post') }}" class="mt-6 space-y-5 form-scroll-lock">
                        ...
                        <x-site.phone-input name="phone" label="{{ __('site.feedback.phone') }}" :value="$prefillPhone" variant="rounded" :required="true" :show-errors="false" />
                        <div>
                    ...
                    <x-site.action-panel open="partnerOpen" :title="__('site.auth.partner_sign_in')" size="md">
                        <form method="POST" action="{{ route('site.login.post') }}" class="space-y-4 form-scroll-lock">
                            @csrf
                            <input type="hidden" name="auth_method" value="password">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('site.auth.email_or_phone') }}</label>
                            ...
                            <div>
                                <div class="flex items-center justify-between mb-1.5">
                                    <label class="block text-sm font-medium text-gray-700">{{ __('site.auth.password') }}</label>
                ...
                @else
                    {{-- Dedicated partner portal route (email/password primary) --}}
                    <form method="POST" action="{{ route('site.login.post') }}" class="mt-6 space-y-5 form-scroll-lock">
                        @csrf
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-100 text-center text-sm text-gray-500 space-y-2">
