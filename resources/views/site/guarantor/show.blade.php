...
<x-site.layout :title="brand_title(__('borrower.guarantor_invite.page_title'))">
    ...
    <div class="max-w-xl mx-auto px-4 py-12">
        ...
        <div class="mb-4 flex items-center justify-between">
            ...
            <span class="text-xs font-semibold uppercase tracking-widest text-amber-700">{{ __('borrower.guarantor_invite.heading') }}</span>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6 sm:p-8">
            ...
            <div class="flex flex-wrap gap-3">
                <form method="POST" action="{{ route('site.guarantor.accept', $invitation->token) }}"
                    ...
                    @csrf
                    <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold px-5 py-2.5 rounded-full text-sm">{{ __('borrower.guarantor_invite.accept') }}</button>
                </form>
                <form method="POST" action="{{ route('site.guarantor.reject', $invitation->token) }}"
                    ...
                    @csrf
                    <button type="submit" class="bg-white ring-1 ring-gray-200 hover:bg-gray-50 text-gray-700 font-semibold px-5 py-2.5 rounded-full text-sm">{{ __('borrower.guarantor_invite.decline') }}</button>
                </form>
            </div>
        </div>
    </div>
