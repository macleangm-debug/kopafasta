<x-site.borrower-layout
    ...
    >
        <x-slot:actions>
            <span class="inline-flex text-xs font-semibold rounded-full px-3 py-1.5 {{ $needsProfile ? 'bg-amber-100 text-amber-900' : 'bg-sky-100 text-sky-800' }}">
                {{ $row->stage_label ?? ($row->application_status['label'] ?? '—') }}
            </span>
        </x-slot:actions>
    </x-site.borrower-page-header>
    ...
    @elseif ($row->pending_hint && ! ($row->is_disbursed ?? false))
        <div class="mb-6 glass-card overflow-hidden ring-1 ring-brand/15">
            <div class="px-5 sm:px-6 py-4">
                <p class="text-[10px] uppercase tracking-widest text-brand font-semibold">{{ __('borrower.guaranteed.whats_pending_title') }}</p>
                <p class="text-sm text-gray-700 mt-1">{{ $row->pending_hint }}</p>
            </div>
        </div>
    @endif
    ...
    <div class="mb-2 glass-card overflow-hidden ring-1 ring-brand/15">
        <div class="px-5 sm:px-6 py-4">
            ...
            <p class="text-sm text-gray-700 mt-1">{{ __('borrower.guaranteed.responsibility_body') }}</p>
        </div>
    </div>
