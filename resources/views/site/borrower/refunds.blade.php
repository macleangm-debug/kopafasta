<x-site.borrower-layout :title="brand_title(__('borrower.refunds_page.title'))" active="refunds" content-width="wide">
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-bold">{{ __('borrower.refunds_page.title') }}</h1>
        <p class="text-sm text-gray-500 mt-1">{{ __('borrower.refunds_page.subtitle') }}</p>
    </div>
    ...
    @if ($refunds->isEmpty())
        <div class="rounded-2xl border border-gray-200 bg-white p-8 text-center text-sm text-gray-500">
            {{ __('borrower.refunds_page.empty') }}
        </div>
    @else
        <div class="space-y-4">
                        ...
                        <form method="POST" action="{{ route('site.borrower.refunds.details', $refund) }}" class="space-y-4 border-t border-gray-100 pt-4" x-data="{ channel: '{{ old('payout_channel', $refund->payout_channel ?? 'mobile_money') }}' }">
                            ...
                            <div class="grid sm:grid-cols-2 gap-3">
                                <label class="cursor-pointer">
                                    ...
                                    <div class="rounded-xl border-2 border-gray-200 peer-checked:border-amber-500 peer-checked:bg-amber-50 p-4 text-sm font-semibold">{{ __('borrower.payments_page.refund.mobile_money') }}</div>
                                </label>
                                <label class="cursor-pointer">
                                    <input type="radio" name="payout_channel" value="bank" x-model="channel" class="sr-only peer">
                                    <div class="rounded-xl border-2 border-gray-200 peer-checked:border-amber-500 peer-checked:bg-amber-50 p-4 text-sm font-semibold">{{ __('borrower.payments_page.refund.bank') }}</div>
                                </label>
                            </div>
