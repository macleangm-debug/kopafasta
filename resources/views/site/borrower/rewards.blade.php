        ...
        <div class="grid gap-6 lg:grid-cols-3">
            <section class="lg:col-span-2 space-y-6">
                <div class="glass-card p-6 sm:p-8">
                    ...
                    <p class="mt-2 text-4xl font-black text-gray-900 tabular-nums">{{ number_format($balance) }}</p>
                ...
                <section class="glass-card p-6">
                    ...
                    <div class="mt-5 grid sm:grid-cols-2 gap-4">
                        @foreach ($catalog as $option)
                            <form method="POST" action="{{ route('site.borrower.rewards.redeem') }}" class="rounded-xl ring-1 ring-gray-200 bg-white p-4 flex flex-col">
                                @csrf
                                ...
                                <button type="submit"
                                        @disabled($balance < $option['points'])
                                        class="mt-3 w-full text-center text-sm font-semibold px-4 py-2.5 rounded-xl {{ $balance >= $option['points'] ? 'bg-brand hover:bg-brand-light text-white' : 'bg-gray-100 text-gray-400 cursor-not-allowed' }}">
            ...
            <aside class="space-y-6">
                <section class="glass-card p-6">
                    ...
                    <a href="{{ route('site.borrower.referrals') }}" class="mt-4 block text-center text-sm font-semibold text-brand hover:underline">
