<x-site.borrower-layout :title="brand_title($payment->reference)" active="payments">
    ...
    @php
        ...
        $collectError = session('collect_error')
    ...
    <div class="max-w-xl mx-auto space-y-5"
         @if ($showCollectFailed && $collectError)
         x-data
         x-init="window.dispatchEvent(new CustomEvent('open-feedback-default', {
             detail: {
                 title: @js(__('borrower.payment_waiting.operator_title')),
                 message: @js($collectError),
                 tone: 'warning',
                 okLabel: @js(__('borrower.payment_waiting.change_phone')),
             }
         }))"
         @endif>
    ...
    @elseif ($isReadyToPay)
        ...
        <div class="rounded-2xl ring-1 ring-brand/10 bg-white px-5 py-5 space-y-4"
             x-data="{ changePhone: {{ $errors->has('mobile_number') || $errors->has('payment_phone') ? 'true' : 'false' }} }">
            <div>
