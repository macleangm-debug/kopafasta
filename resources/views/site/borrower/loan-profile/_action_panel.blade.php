    ...
    <div class="mb-6 glass-card overflow-hidden ring-1 ring-brand/10">
        ...
        <div class="px-5 sm:px-6 py-5">
            ...
            @if ($underwritingActions->isNotEmpty())
                ...
                @endphp
                <div id="underwriting-requests" class="mt-1 rounded-xl bg-amber-50 ring-1 ring-amber-200/80 px-4 py-4"
                    ...
                    <div class="mt-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                        <p class="text-sm font-semibold text-gray-900 min-w-0">{{ $primaryUw['label'] }}</p>
                        <a href="{{ in_array($primaryUw['kind'] ?? '', ['document', 'clarification'], true) ? '#documents' : ($primaryUw['url'] ?? '#documents') }}"
                        ...
                        <div x-show="open" x-cloak class="mt-3 flex flex-col gap-2">
                            @foreach ($extraUw as $action)
                                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 rounded-lg bg-white/90 ring-1 ring-amber-100 px-3 py-2.5">
                                    <p class="text-sm font-semibold text-gray-900 min-w-0">{{ $action['label'] }}</p>
                                    <a href="{{ $action['url'] }}"
