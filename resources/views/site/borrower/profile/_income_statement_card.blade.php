...
<x-site.profile-section-card
    ...
    <x-slot:view>
        <div data-kf-doc-view="profile-income-statement"
             data-kf-doc-codes="bank_statement mobile_money_statement mpesa_statement salary_slip"
             data-kf-doc-view-label="{{ __('borrower.profile.view_document') }}"
             class="space-y-4">
            <div data-kf-doc-ssr @class(['space-y-4' => $hasStatement, 'hidden' => ! $hasStatement])>
                @if ($hasStatement)
            ...
            {{-- Filled immediately after AJAX upload (no reload). --}}
            <div data-kf-live-docs class="space-y-4 hidden"></div>

            <div data-kf-doc-empty @class(['hidden' => $hasStatement])>
                <p class="text-sm text-gray-600">{{ __('borrower.profile.income_statement_card_hint') }}</p>
                <button type="button" @click="open = true; $dispatch('income-statement-reset')"
                    ...
                    {{ __('borrower.profile.income_add_statement') }}
                </button>
            </div>

            @unless ($incomeProofEmployed)
                <div data-kf-doc-add-another @class(['hidden' => ! $hasStatement])>
                    <button type="button"
                            @click="open = true; $dispatch('income-statement-reset')"
