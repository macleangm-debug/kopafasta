<x-site.borrower-layout :title="brand_title(__('borrower.loan_actions.restructure_title'))" active="loans" content-width="wide">
    <div>
        ...
        @else
            <form method="post" action="{{ route('site.borrower.loans.restructure.submit', $loan) }}" class="glass-card p-6 space-y-5" x-data="{ type: @js(old('restructure_type', '')) }">
                @csrf
                ...
                <div>
                    ...
                    <select name="restructure_type" required x-model="type" class="w-full rounded-lg border-gray-300 ring-1 ring-gray-200 px-3 py-2.5 text-sm focus:ring-brand">
