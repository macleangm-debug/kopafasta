...
<div class="mb-6" x-data="{ sectionsOpen: false }">
    ...
    <div class="hidden lg:block relative" @click.outside="sectionsOpen = false">
        <div class="flex items-center justify-between gap-3">
            ...
            @else
                <p class="text-sm font-semibold text-gray-500">{{ __('site.partner_account.sections_title') }}</p>
            @endif
        ...
        <div x-show="sectionsOpen" x-cloak x-transition
            ...
            <p class="px-3 py-2 text-[10px] uppercase tracking-widest font-bold text-gray-500">{{ __('site.partner_account.sections_title') }}</p>
            @foreach ($tabs as $key => $label)
                @php
                    $isActive = $active === $key;
                    $status = $partner ? $service->sectionStatus($partner, $key) : null;
