<div x-show="stepKey === 'guarantor'" class="p-6 sm:p-8">
    ...
    <div class="space-y-5" x-show="!isGuarantorLocked()">
        ...
        <div class="glass-card rounded-2xl ring-1 ring-gray-200/80 p-1.5 flex flex-wrap gap-1" x-show="form.guarantor_mode !== 'previous'">
            <label class="flex-1 min-w-[10rem]">
                <input type="radio" name="guarantor_mode" value="internal" x-model="form.guarantor_mode" class="sr-only peer">
                <span class="block text-center text-sm font-semibold px-4 py-2.5 rounded-xl cursor-pointer transition peer-checked:bg-brand peer-checked:text-white text-gray-600 hover:bg-gray-50">{{ __('borrower.apply.internal_guarantor') }}</span>
            </label>
            <label class="flex-1 min-w-[10rem]">
                <input type="radio" name="guarantor_mode" value="external" x-model="form.guarantor_mode" class="sr-only peer">
                <span class="block text-center text-sm font-semibold px-4 py-2.5 rounded-xl cursor-pointer transition peer-checked:bg-brand peer-checked:text-white text-gray-600 hover:bg-gray-50">{{ __('borrower.apply.external_guarantor') }}</span>
            </label>
        </div>
