<x-site.investor-layout title="{{ $pool->name }} — Pool" active="pools">
    <a href="{{ route('site.investor.pools') }}" class="text-sm text-brand hover:underline font-semibold">← All pools</a>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-4">
        <div class="lg:col-span-2 glass-card rounded-2xl ring-1 ring-brand/10 p-6">
            <div class="flex items-start justify-between gap-3 mb-4">
                <div>
                    <p class="text-xs uppercase tracking-wider text-gray-500 font-semibold">{{ ucfirst($pool->pool_type) }} loans</p>
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-6">
                <div class="rounded-lg bg-brand-muted/30 p-3">
                    <p class="text-[11px] uppercase text-gray-500 font-semibold">Expected yield</p>
                    <p class="text-xl font-extrabold text-brand">{{ rtrim(rtrim(format_number($pool->expected_yield, 2),'0'),'.') }}%</p>
                </div>
                <div class="rounded-lg bg-brand-muted/30 p-3">
                    <p class="text-[11px] uppercase text-gray-500 font-semibold">Repayment rate</p>
                    <p class="text-xl font-extrabold">{{ rtrim(rtrim(format_number($pool->repayment_rate, 1),'0'),'.') }}%</p>
                </div>
                <div class="rounded-lg bg-brand-muted/30 p-3">
                    <p class="text-[11px] uppercase text-gray-500 font-semibold">Default rate</p>
                    <p class="text-xl font-extrabold {{ $pool->default_rate > 5 ? 'text-red-600' : '' }}">{{ rtrim(rtrim(format_number($pool->default_rate, 1),'0'),'.') }}%</p>
                </div>
                <div class="rounded-lg bg-brand-muted/30 p-3">
                    <p class="text-[11px] uppercase text-gray-500 font-semibold">Active borrowers</p>
            ...
            <div class="mt-6 grid grid-cols-2 gap-3 text-sm">
                <div><span class="text-gray-500">Currency:</span> <span class="font-semibold">{{ $pool->currency }}</span></div>
                <div><span class="text-gray-500">Status:</span> <span class="font-semibold capitalize">{{ $pool->status }}</span></div>
                <div><span class="text-gray-500">Opens:</span> <span class="font-semibold">{{ optional($pool->start_date)->format('d M Y') ?: '—' }}</span></div>
                <div><span class="text-gray-500">Matures:</span> <span class="font-semibold">{{ optional($pool->end_date)->format('d M Y') ?: '—' }}</span></div>
            </div>
        </div>

        <div class="glass-card rounded-2xl ring-1 ring-brand/10 p-6 h-fit sticky top-20">
            <h2 class="font-bold mb-1">Invest in this pool</h2>
            <p class="text-xs text-gray-500 mb-4">Your available balance: <span class="font-semibold">TZS {{ $fmt($stats['available']) }}</span></p>
            ...
            <form method="POST" action="{{ route('site.investor.pool.invest', $pool) }}" class="space-y-3">
                ...
                <div class="rounded-lg bg-emerald-50 border border-emerald-200 p-3 text-xs text-brand">
                ...
                @if ($stats['available'] <= 0)
                    <a href="{{ route('site.investor.wallet') }}" class="block text-center text-xs text-brand hover:underline font-semibold">Deposit funds first →</a>
                @endif
            </form>
        </div>
    </div>
