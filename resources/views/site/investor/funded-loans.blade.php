<x-site.investor-layout title="Funded loans — Investor" active="funded">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="glass-card rounded-xl ring-1 ring-brand/10 p-4 shadow-sm">
            ...
            <p class="text-xl font-bold text-gray-900 mt-1">TZS {{ number_format($capitalMetrics['capital_invested'], 0) }}</p>
        </div>
        <div class="glass-card rounded-xl ring-1 ring-brand/10 p-4 shadow-sm">
            <p class="text-xs uppercase tracking-wider text-gray-500 font-semibold">Outstanding exposure</p>
        ...
        <div class="rounded-xl border ring-1 ring-brand/20 bg-brand-muted/40 p-4 shadow-sm">
            <p class="text-xs uppercase tracking-wider text-brand font-semibold">Interest earned (you)</p>
            <p class="text-xl font-bold text-brand mt-1">TZS {{ number_format($capitalMetrics['interest_earned_partner'], 0) }}</p>
            <p class="text-[10px] text-brand mt-1">{{ \App\Services\CapitalPartnerAllocationService::PARTNER_INTEREST_SHARE }}% partner share</p>
        </div>
        <div class="glass-card rounded-xl ring-1 ring-brand/10 p-4 shadow-sm">
            <p class="text-xs uppercase tracking-wider text-gray-500 font-semibold">Active loans</p>
            <p class="text-xl font-bold text-gray-900 mt-1">{{ $capitalMetrics['active_loans'] }}</p>
        </div>
    </div>

    <div class="glass-card rounded-2xl ring-1 ring-brand/10 shadow-sm overflow-hidden">
        @if ($allocations->isEmpty())
            <p class="p-8 text-sm text-gray-500 text-center">No loan allocations yet. Capital is deployed when Finance disburses approved loans.</p>
        @else
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-brand-muted/30 text-xs uppercase tracking-wider text-gray-500">
                        <tr>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @foreach ($allocations as $row)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-right font-mono text-brand">{{ format_money($row['interest_earned_partner']) }}</td>
