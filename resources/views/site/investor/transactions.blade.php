<x-site.investor-layout title="Transactions — Investor" active="transactions">
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-slate-500 text-sm mt-1">Every deposit, withdrawal, investment, payout and fee.</p>
        </div>
    </div>
    ...
    <div class="flex flex-wrap gap-2 mb-4">
        @foreach (['' => 'All', 'deposit' => 'Deposits', 'withdrawal' => 'Withdrawals', 'investment' => 'Investments', 'return' => 'Returns', 'fee' => 'Fees'] as $k => $label)
            <a href="?type={{ $k }}" class="rounded-full px-3 py-1.5 text-xs font-semibold border
                {{ $type === $k ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-700 border-slate-300 hover:bg-slate-50' }}">{{ $label }}</a>
        @endforeach
    </div>

    <div class="rounded-2xl border border-slate-200 bg-white overflow-hidden">
        @if ($transactions->isEmpty())
            <div class="p-10 text-center text-slate-500 text-sm">No transactions yet.</div>
        @else
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach ($transactions as $t)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-4 py-3 text-right font-semibold {{ in_array($t->type, ['return','deposit']) ? 'text-emerald-700' : '' }}">TZS {{ $fmt($t->amount) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
