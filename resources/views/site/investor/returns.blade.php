...
<x-site.investor-layout title="Returns & earnings — Investor" active="returns">
    ...
    <p class="text-gray-500 text-sm mb-6">Track every shilling earned and what's still expected.</p>
    ...
    <div class="glass-card rounded-2xl ring-1 ring-brand/10 p-6 mb-6">
        ...
        @if ($monthly->isEmpty())
            <p class="text-sm text-gray-500">No payout history yet.</p>
        @else
            <div class="flex items-end gap-3 h-48">
                @foreach ($monthly as $m)
                    ...
                    <div class="flex-1 flex flex-col items-center gap-2">
                        ...
                        <span class="text-[10px] text-gray-500 font-medium">{{ \Illuminate\Support\Carbon::createFromFormat('Y-m', $m->ym)->format('M') }}</span>
                    </div>
                @endforeach
            </div>
        @endif
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach ($byPool as $name => $d)
                        <tr>
                            ...
                            <td class="py-2 text-right text-brand font-semibold">TZS {{ $fmt($d['returns']) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
