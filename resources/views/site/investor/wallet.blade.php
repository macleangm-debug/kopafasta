    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            <div class="rounded-2xl bg-gradient-to-br from-slate-900 to-emerald-900 text-white p-6 shadow-lg">
                <p class="text-xs uppercase tracking-widest text-emerald-300/80 font-semibold">Available balance</p>
                <p class="text-4xl font-extrabold mt-1">TZS {{ $fmt($stats['available']) }}</p>
                <div class="grid grid-cols-3 gap-4 mt-5">
                    <div><p class="text-xs text-emerald-300/80 uppercase">Total deposited</p><p class="font-bold">TZS {{ $fmt($stats['deposited']) }}</p></div>
                    <div><p class="text-xs text-emerald-300/80 uppercase">Total withdrawn</p><p class="font-bold">TZS {{ $fmt($stats['withdrawn']) }}</p></div>
                    <div><p class="text-xs text-emerald-300/80 uppercase">Earned</p><p class="font-bold text-emerald-400">TZS {{ $fmt($stats['returnsPaid']) }}</p></div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="rounded-2xl border border-slate-200 bg-white p-6">
                    ...
                    <p class="text-xs text-slate-500 mb-4">Add capital ready for investment.</p>
                    <form method="POST" action="{{ route('site.investor.wallet.deposit') }}" class="space-y-3">
                        ...
                        <div>
                            ...
                            <select name="channel" required class="w-full mt-1 rounded-lg border border-slate-300 px-3 py-2 text-sm">
                        ...
                        <div>
                            <label class="text-xs uppercase text-slate-500 font-semibold">Payment reference (optional)</label>
                            <input type="text" name="payment_reference" maxlength="80" class="w-full mt-1 rounded-lg border border-slate-300 px-3 py-2 text-sm" />
                        </div>
                ...
                <div class="rounded-2xl border border-slate-200 bg-white p-6">
                    ...
                    <p class="text-xs text-slate-500 mb-4">Cash out your available balance.</p>
                    <form method="POST" action="{{ route('site.investor.wallet.withdraw') }}" class="space-y-3">
                        ...
                        <div>
                            <label class="text-xs uppercase text-slate-500 font-semibold">Destination reference</label>
                            <input type="text" name="payment_reference" maxlength="80" placeholder="Account or wallet number" class="w-full mt-1 rounded-lg border border-slate-300 px-3 py-2 text-sm" />
                        </div>
                ...
                <ul class="space-y-3 text-sm">
                    @foreach ($recent as $t)
                        <li class="flex items-center justify-between border-b border-slate-100 pb-2 last:border-0">
                            ...
                            <p class="font-semibold {{ in_array($t->type, ['return','deposit']) ? 'text-emerald-700' : '' }}">TZS {{ $fmt($t->amount) }}</p>
                        </li>
                    @endforeach
                </ul>
            @endif
