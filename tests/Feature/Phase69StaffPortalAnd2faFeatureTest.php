...
use App\Services\AuthPortalSettingsService;
...
class Phase69StaffPortalAnd2faFeatureTest extends TestCase
{
    ...
    public function test_trusted_device_cookie_does_not_skip_admin_2fa_challenge(): void
    {
        ...
        $cookie = app(TrustedDeviceService::class)->makeCookie($token);
    ...
    {
        Setting::set('auth_portal.require_2fa_admin', false);
        ...
        $this->actingAs($admin, 'admin')
            ->put(route('admin.settings.auth-portal.save'), [
                ...
                'pin_recovery_session_seconds' => 90,
            ])
            ->assertRedirect()
            ->assertSessionHas('status');

        $this->assertFalse(app(AuthPortalSettingsService::class)->require2faStaff());
        $this->assertTrue(app(AuthPortalSettingsService::class)->require2faAdmin());
        $this->assertSame(8, app(AuthPortalSettingsService::class)->twoFactorSessionHours());
